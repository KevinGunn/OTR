...
double_cv.ks <- function(Yt.in , x.in , const_in, prop_score){
  ...
  for (fold in 1:num.folds) {
    ...
    beta_k = beta_cv.ks(y.train,x.train[c("X1","X2")],const_in = const_in, 
                        prop_score = prop_score.train )
    ...
    #beta.off = offlm.model$coefficients
    mu_hat = as.vector(mx.hat + beta_k%*%t(x.test))
...
IF_sd <- function(XtX.inv,X,Y,A,mv,prop ){
  ...
  #print("Varm1,Varm0");print(V1);print(V0)
  return(SE)
}
...
# IF with 5/4 *n.
IF_sd2 <- function(XtX.inv,X,Y,A,mv,prop ){
...
Sim_OTR <- function(n , N , p=2, model = "SL" ,lambda=1,beta=NULL,g=NULL,num.sims=500){
  ...
  for(sim in 1:num.sims){
    ...
    MSE.ols_b0[sim] = sum((beta.OLS[1] - beta.t[3])^2)
    MSE.ols_b1[sim] = sum((beta.OLS[2] - beta.t[3])^2)
    MSE.ols_b2[sim] = sum((beta.OLS[3] - beta.t[3])^2)
    
    MSE.CXls_b0[sim] = sum((beta.CX_ls[1] - beta.t[3])^2)
    MSE.CXls_b1[sim] = sum((beta.CX_ls[2] - beta.t[3])^2)
    ...
    #RF.in = c(predict(RF.model_A1,newdata=X.l_1),predict(RF.model_A0,newdata=X.l_0))
    ####################################################################################################
    
    print(beta.CX_ls)
    IF.CXls_sd = IF_sd(GammaInv, X.in, Y.in, A.in, mv.input,prop.in )
    print(IF.CXls_sd)
    #IF.CXls_sd = IF_sd2(GI1,GI0, X.in, Y.in, A.in, mv.input,prop.in )
    #print(IF.CXls_sd)
...
capture.output(Sim_L2L,file="MCAR_L2L.txt")
capture.output(Sim_QL,file="MCAR_CL.txt")
capture.output(Sim_L2NL3,file="MCAR_L2NL3.txt")
capture.output(Sim_QNL3,file="MCAR_CNL3.txt")
capture.output(Sim_L2S,file="MCAR_L2S.txt")
capture.output(Sim_QS,file="MCAR_CS.txt")
