...
###########################################################################################
###########################################################################################
score <- function(X, A, beta.log){
  score_vec = as.vector( t(X)%*% (A-plogis(X%*%beta.log)) )
  return(score_vec)
}
hess <- function(X, A, beta.log){
  varp = plogis(X%*%beta.log)*(1- plogis(X%*%beta.log))
  XtWX = t(X) %*% diag(as.vector(varp)) %*% X   
  return(solve(XtWX))
}
prop_deriv <- function(X, A, beta.log){
  
  pd = exp(X%*%beta.log) / (1 + exp(X%*%beta.log) )^2
  deriv = t(pd) %*% X
  return(as.matrix(deriv))
}
IF_deriv <- function(X, A, Y,prop, mv, prop_d ){
  
  p = dim(X)[2]
  IF1d_update = matrix(0,ncol=p,nrow=p)
  IF0d_update = matrix(0,ncol=p,nrow=p)
  n = length(Y)
  ...
  for(i in 1:n){
    ...
    if(A[i]==1){
      # Variance  of IF
      #print((Y[i] - mv[i]))
      IF1 = as.matrix(X[i,]*(Y[i]- mv[i])*prop[i]^-2)
      IF1_d = ( IF1%*%prop_d ) 
      IF1d_update = IF1d_update + IF1_d
    } else{
      #print((Y[i] - mv[i]))
      IF0 = as.matrix(X[i,]*(Y[i]- mv[i])*(1-prop[i])^-2) 
      IF0_d = ( IF0%*%prop_d ) 
      IF0d_update = IF0d_update + IF0_d
    } 
    
  }
  
  return(IF1d_update + IF0d_update)
...
IF_est_sd <- function(XtX.inv, X, Y, A, mv, prop, prop_eqn ){
  ...
  for(i in 1:n){
    ...
    if(A[i]==1){
      # Variance  of IF
      #print((Y[i] - mv[i]))
      IF1 = X[i,]*(Y[i]- mv[i])*prop[i]^-1  - prop_eqn
      Var_IF1 = ( IF1%*%t(IF1) ) 
      varm1 = varm1 + Var_IF1
    } else{
      #print((Y[i] - mv[i]))
      IF0 = -X[i,]*(Y[i]- mv[i])*(1-prop[i])^-1 - prop_eqn
      Var_IF0 = ( IF0%*%t(IF0) ) 
      varm0 = varm0 + Var_IF0
    } 
...
mn_bstrap_var <- function(X.l, Y, A, prop_score, X.U, numB, hc1,hc0, m ){
...
#########################################################################################
bstrap_var <- function(X.l, Y, A, X.U, numB, hc1,hc0 ){
  ...
  for( b in 1:numB){
    ...
    # Bootstrap sample for observed cases.
    samp.l = sample.int(n=n.l,size=n.l,replace=TRUE)
    X.lb = X.l[samp.l,] ; Yb = Y[samp.l] ; Ab = A[samp.l]
...
MC_coeff <-function(model = "L2L", n.samp = 300000,p,g=NULL,theta,lambda,beta){
  ...
  if(model == "QL"){
    ...
    Y = as.vector( X%*%beta*(1+X%*%theta) + A*lambda*(X%*%beta) + rnorm(n.samp,0,1))
    mu_X = X%*%beta; CX = X%*%beta
  }else if(model == "L2NL3"){
    Y = as.vector(X%*%beta*(1+X%*%theta) + A*lambda*((X%*%g)^3) + rnorm(n.samp,0,1))
    mu_X = X%*%beta; CX = (X%*%g)^3
  }else if(model == "QNL3"){
    Y = as.vector((X%*%theta)^2 + A*lambda*((X%*%g)^3) + rnorm(n.samp,0,1))
    mu_X = (X%*%theta)^2; CX = (X%*%g)^3
  }else if(model == "L2S"){
    Y = as.vector(X%*%beta*(1+X%*%theta) + A*lambda*(sin(X%*%beta) ) + rnorm(n.samp,0,1))
...
Sim_OTR <- function(n , N , p=2, model = "SL" ,lambda=1,beta=NULL,g=NULL,num.sims=500){
  ...
  for(sim in 1:num.sims){
    ...
    #print(summary(X.l_1));print(summary(X.l_0));print(summary(X.i_KS))
    #print(summary(prop1));print(summary(prop0))
    #Predictions with bias reduction by least squares
    ...
    ####################################################################################################
    # get estimating eqn for prop score.
    sv = score(X.in,A.in,prop_model$coefficients)
    hess_mat = hess(X.in, A.in, prop_model$coefficients)
    pd = prop_deriv(X.in,A.in, prop_model$coefficients)
    IF_d = IF_deriv(X.in,A.in,Y.in,mv.input,prop.in,pd)
    
    prop_eqn = IF_d %*% hess_mat %*% sv
    
    ####################################################################################################
    
    #IF.CXls_sd = IF_sd(GammaInv, X.in, Y.in, A.in, mv.input, prop.in )
    IF.CXls_sd = IF_est_sd(GammaInv, X.in, Y.in, A.in, mv.input, prop.in, prop_eqn )
    print(beta.CX_ls)
    print(IF.CXls_sd)
    #IF.CXls_sd = IF_sd2(GammaInv, X.in, Y.in, A.in, mv.input1, mv.input0,prop.in )
...
#p=10
beta.in = rep(1,2);g.in = c(0.3,0.6); g_large = c(0.2,0.4)
###################################################################################
Sim_L2L = Sim_OTR(n=1000,N=10000, p=2, model = "L2L",beta = beta.in,num.sims = 500)
#print(Sim_LL)

Sim_QL = Sim_OTR(n=1000,N=10000, p=2, model = "QL",beta = beta.in,num.sims=500)
print(Sim_QL$bias_sd_CXls)

Sim_L2NL3 = Sim_OTR(n=1000,N=10000, p=2, model = "L2NL3",beta=beta.in,g = g.in,num.sims=500)

Sim_QNL3 = Sim_OTR(n=1000,N=10000, p=2, model = "QNL3",beta=NULL,g = g.in,num.sims= 500)

Sim_L2S = Sim_OTR(n=1000,N=10000, p=2, model = "L2S",beta=beta.in, num.sims=500)

Sim_QS = Sim_OTR(n=1000,N=10000, p=2, model = "QS",beta=beta.in, num.sims=500)

capture.output(Sim_L2L,file="new_IF_MCAR_L2L.txt")
capture.output(Sim_QL,file="new_IF_MCAR_QL.txt")
capture.output(Sim_L2NL3,file="new_IF_MCAR_L2NL3.txt")
capture.output(Sim_QNL3,file="new_IF_MCAR_QNL3.txt")
capture.output(Sim_L2S,file="new_IF_MCAR_L2S.txt")
capture.output(Sim_QS,file="new_IF_MCAR_QS.txt")
