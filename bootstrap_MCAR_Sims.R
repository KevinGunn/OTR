...
# Influence Function
IF_sd <- function(XtX.inv,X1,X0,Y1,Y0,mv1,mv0,prop1,prop0){
  ...
  for(i in 1:n1){
    ...
    IF1 = XtX.inv%*%X1[i,]%*%(Y1[i] - mv1[i])
    
    Var_IF1 = IF1%*%t(IF1)
    
    varm1 = (1/prop1[i])*Var_IF1 + varm1
    
  }
  
  for(i in 1:n0){
    
    IF0 = XtX.inv%*%X0[i,]%*%(Y0[i] - mv0[i])
    
    Var_IF0 = IF0%*%t(IF0)
    
    varm0 = (1/(1-prop0[i]))*Var_IF0 + varm0
    
  }
  
  varm = ( (1/n1)*varm1 + (1/n0)*varm0 ) /n 
  #n10 = 1 #(n1 + n0)/(n1*n0)
  return(sqrt(diag(varm)))
}

IF_sd2 <- function(XtX.inv,X1,X0,Y1,Y0,mv1,mv0,prop1,prop0){
  ...
  varm = (1/n1^2)*varm1 + (1/n0^2)*varm0 
  sd = sqrt(diag(varm)) #* sqrt(n1^-1 + n0^-1 )
  return(sd)
}

IF_sd3 <- function(XtX.inv,X,Y,A,mv,prop){
...
IF_sd4 <- function(XtX.inv,X,Y,A,mv,prop,double_CX,beta_est){
  ...
  for(i in 1:n){
    ...
    if(A[i]==1){
      # Variance  of IF
      #print((Y[i] - mv[i]))
      IF1 = prop[i]^(-1)*X[i,]*(Y[i] - mv[i])
      Var_IF1 = ( IF1%*%t(IF1) )
      varm = Var_IF1 + varm
      #IF_b = X[i,]*(as.vector(Y[i]) - as.numeric(t(X[i,])%*%beta_est) )
    } else{
      #print((Y[i] - mv[i]))
      IF0 = (1-prop[i])^(-1)*X[i,]*(Y[i] - mv[i])
      Var_IF0 = ( IF0%*%t(IF0) )
      varm = Var_IF0 + varm
      #IF_b = X[i,]*(as.vector(Y[i]) - as.vector(t(X[i,])%*%beta_est) )
    }
    IF_b = X[i,]*(double_CX[i] - as.numeric(t(X[i,])%*%beta_est) )
    V_IFb = IF_b %*% t(IF_b)
    varb = V_IFb + varb
  }
  
  var = XtX.inv%*%(varm + varb)%*%t(XtX.inv) / n^2
  se = sqrt(diag(var))
  #print(se)
  return(se)
}

IF_sd5 <- function(XtX.inv,X,Y,A,mv,prop,u1,u0,beta_est){
  ...
  for(i in 1:n){
    ...
    if(A[i]==1){
      ...
      Var_IF1 = ( IF1%*%t(IF1) )
      varm = Var_IF1 + varm
      IF_b = X[i,]*( Y[i] - u0[i] - as.numeric(t(X[i,])%*%beta_est) )
    } else{
      #print((Y[i] - mv[i]))
      IF0 = (1-prop[i])^(-1)*X[i,]*(Y[i] - mv[i])
      Var_IF0 = ( IF0%*%t(IF0) )
      varm = Var_IF0 + varm
      IF_b = X[i,]*(u1[i] - Y[i] - as.numeric(t(X[i,])%*%beta_est) ) 
    }
    #IF_b = X[i,]*(double_CX[i] - as.numeric(t(X[i,])%*%beta_est) )
    V_IFb = IF_b %*% t(IF_b)
    varb = V_IFb + varb
...
#########################################################################################
bstrap_var <- function(X.l, Y, A, prop_score, X.U, numB, hc1,hc0 ){
  ...
  for( b in 1:numB){
    ...
    samp.l = sample.int(n=n.l,size=0.5*n.l,replace=TRUE)
    X.lb = X.l[samp.l,] ; Yb = Y[samp.l] ; Ab = A[samp.l]
    propb = prop_score[samp.l]
    
    # Bootstrap sample for incomplete cases.
    samp.u = sample.int(n=n.u,size=0.5*n.u,replace=TRUE)
    X.Ub = X.U[samp.u,]
...
MC_coeff <-function(model = "SL", n.samp = 300000,p,g=NULL,theta,lambda,beta){
  ...
  true.prop = plogis(sqrt(X[,1]^2)+sqrt(X[,2]^2)-1.5)
  ...
  if(model == "SL"){
    Y = as.vector(sin(X%*%beta)+0.2*(X%*%theta)^2 + A*lambda*(X%*%beta) + rnorm(n.samp,0,1))
    mu_X = sin(X%*%beta)+0.2*(X%*%theta)^2; CX = X%*%beta
  }else if(model == "QL"){
    ...
    Y = as.vector(sin(X%*%beta)+0.2*(X%*%theta)^2 + A*lambda*((X%*%g)^3) + rnorm(n.samp,0,1))
    mu_X = sin(X%*%beta)+0.2*(X%*%theta)^2; CX = (X%*%g)^3
  }else if(model == "QNL3"){
    Y = as.vector((X%*%theta)^2 + A*lambda*((X%*%g)^3) + rnorm(n.samp,0,1))
    mu_X = (X%*%theta)^2; CX = (X%*%g)^3
  }else if(model == "SS"){
    Y = as.vector(sin(X%*%beta)+0.2*(X%*%theta)^2 + A*lambda*(sin(X%*%beta) ) + rnorm(n.samp,0,1))
    mu_X = sin(X%*%beta)+0.2*(X%*%theta)^2; CX = sin(X%*%beta)
  }else if(model == "QS"){
...
#n = 500;N=3000;p=2;mu_0 = 1;g=1;g2=1;lambda=1;model = "QL";beta = c(1,1);num.sims=50
...
Sim_OTR <- function(n , N , p=2, model = "SL" ,lambda=1,beta=NULL,g=NULL,num.sims=500){
  ...
  for(sim in 1:num.sims){
    ...
    #Generate Y based on the given true model.
    if(model == "SL"){
      Y = as.vector(sin(X%*%beta)+0.2*(X%*%theta)^2 + A*lambda*(X%*%beta) + rnorm(n+N,0,1))
      CX = X%*%beta
    }else if(model == "QL"){
      Y = as.vector((X%*%theta)^2 + A*lambda*(X%*%beta) + rnorm(n+N,0,1))
      CX = X%*%beta
    }else if(model == "LL"){
      Y = as.vector( X%*%beta + A*lambda*(X%*%beta) + rnorm(n+N,0,1))
      mu_X = X%*%beta ; CX = X%*%beta
    }else if(model == "SNL3"){
      Y = as.vector(sin(X%*%beta)+0.2*(X%*%theta)^2 + A*lambda*((X%*%g)^3) + rnorm(n+N,0,1))
      CX = (X%*%g)^3
    }else if(model == "QNL3"){
      Y = as.vector((X%*%theta)^2 + A*lambda*((X%*%g)^3) + rnorm(n+N,0,1))
      CX = (X%*%g)^3
    }else if(model == "SS"){
      Y = as.vector(sin(X%*%beta)+0.2*(X%*%theta)^2 + A*lambda*(sin(X%*%beta) ) + rnorm(n+N,0,1))
      CX = sin(X%*%beta)
    }else if(model == "QS"){
    ...
    #True values for response variable in transformed outcome linear regression.
    Yt.true = Y*(A - true.prop) / (true.prop*(1 - true.prop))
    Yt = Yt.true[1:n]; Y.known = Y[1:n]; A.known = A[1:n];prop = true.prop[1:n];Nprop = true.prop[-(1:n)]
    ...
    IF.CXls_sd = IF_sd3(GammaInv, X.in, Y.in, A.in, mv.input,prop.in )
    ### IF sd 3
    #print(IF.CXls_sd)
    #IF.CXls_sd = IF_sd4(GammaInv, X.in, Y.in, A.in, mv.input,prop.in, 
    #                    double_CX = dcx, beta_est = beta.CX_ls)
    
    #IF.CXls_sd = IF_sd5(GammaInv, X.in, Y.in, A.in, mv.input,prop.in, 
    #                    u1 = u1,u0=u0, beta_est = beta.dcvb)
    
    #IF.CXls_sd = IF_sd(GammaInv,as.matrix(X.l_1),as.matrix(X.l_0),Y_A1,Y_A0,dcv.mx1,dcv.mx0,prop1,prop0)
    #print(IF.CXls_sd)
    #IF.CXls_sd = IF_sd2(GammaInv,as.matrix(X.l_1),as.matrix(X.l_0),Y_A1,Y_A0,dcv.mx1,dcv.mx0,prop1,prop0)
    ...
    ###############################################################
    #Bootstrap
    bVar = bstrap_var( X.l=X.label,Y=Y.known,A=A.known,prop_score=prop,X.U=X.imp,
                       numB=500, hc1=hc1, hc0=hc0 )
    
    boot.mat[sim,] = bVar
    #CX_ls RAL estimator
    lower.cx = beta.CX_ls - z*bVar
    upper.cx = beta.CX_ls + z*bVar
  ...
  RE = list( RE_RF = RE.RF,
             ...
             bias_sd_CXls = cbind(beta.t,bias=beta.CX_ls.mean-beta.t ,Esd=beta.CX_ls.sd,Asd_IF = IF.CXls_avg,CP=CP_CX,
                                  boot_se = boot.CXls_avg,CP_boot = CP_CX_boot),
...
###################################################################################
#Sim_LL = Sim_OTR(n=500,N=3000, p=2, model = "LL",beta = beta.in,num.sims = 5)
#print(Sim_LL)

#Sim_SL = Sim_OTR(n=500,N=3000, p=2, model = "SL",beta = beta.in,num.sims = 500)
#print(Sim_SL)

#Sim_QL = Sim_OTR(n=500,N=3000, p=2, model = "QL",beta = beta.in,num.sims=500)

#Sim_SNL3 = Sim_OTR(n=500,N=3000, p=2, model = "SNL3",beta=beta.in,g = g.in,num.sims=500)

Sim_QNL3 = Sim_OTR(n=500,N=3000, p=2, model = "QNL3",beta=NULL,g = g.in,num.sims= 500)

#Sim_SS = Sim_OTR(n=500,N=3000, p=2, model = "SS",beta=beta.in, num.sims=500)

#Sim_QS = Sim_OTR(n=500,N=3000, p=2, model = "QS",beta=beta.in, num.sims=500)

#capture.output(Sim_SL,file="VAR_MCAR_SL.txt")
#capture.output(Sim_QL,file="VAR_MCAR_QL.txt")
#capture.output(Sim_SNL3,file="VAR_MCAR_SNL3.txt")
capture.output(Sim_QNL3,file="bstrap0_5_MCAR_QNL3.txt")
#capture.output(Sim_SS,file="VAR_MCAR_SS.txt")
